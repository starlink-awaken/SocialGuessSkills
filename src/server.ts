...
function registerTools(mcpServer: McpServer) {
  const trackedTools: string[] = (mcpServer as any).__registeredTools ?? [];
  ...
  trackedTools.push("validate_model");
  ...
  trackedTools.push("health_check");
  (mcpServer as any).__registeredTools = trackedTools;
}
