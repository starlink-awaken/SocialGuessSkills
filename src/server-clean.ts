...
(mcpServer as any).registerTool("query_model_history", queryModelHistoryConfig, async function queryModelHistory(args: QueryModelHistoryArgs, _extra?: any): Promise<any> {
...
(async function main() {
  ...
  const httpServer = createNodeServer((req, res) => {
    ...
    if (req.url === "/metrics") {
      ...
      res.end("# HELP mcp_requests_total Total number of MCP requests processed\n# TYPE mcp_requests_total counter\nmcp_requests_total 0\n");
      return;
    }
  ...
  logger.info({ msg: `HTTP metrics server listening on ${port}`, port });
});
})();
*/